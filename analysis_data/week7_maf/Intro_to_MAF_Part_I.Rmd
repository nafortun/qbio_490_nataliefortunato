...
```{r Exercise 1.1}
clinical <- read.csv("/Users/nataliefortunato/Documents/qbio_490_nataliefortunato/analysis_data/brca_clinical_data.csv")
```
...
write.csv(clinical, "brca_clinical_data.csv", row.names = FALSE)
```
...
Refer to a codon chart and amino acid abbreviation chart to answer the following:
 ...
 Either the final nucleotide being changed from a U or C to an A or G, or changing the first nucleotide from an A to a C.
 - What about an A/T change?
Changing the first nucleotide from a G to an A
...
It is a point mutation (frame shift) at the 11th nucleotid of the P728A gene.
...
Patients positive for the breast carcinoma estrogen receptor are 1.65 times more likely to be alive compared to if their status was negative. There is a 4.35% chance that these results are due to chance, meaning that the results are significant if a significance value of 0.05 was used. 

Does this make sense given the biology of cancer?
Yes because the breast carcinoma estrogen receptor is an easy target for treatment therapies. Therefore, patients with this receptor can recieve more effective treatment which increases their likelyhood of survival.
...
They are co-occurent because they occur simultaneously in many instances.
...
ggsave("/Users/nataliefortunato/Documents/qbio_490_nataliefortunato/analysis_data/week7_maf/oncoplot.png")
```
...
Of the patients with the top ten gene mutations, more of them were young than old.
...
coOncoplot(m1 = young_maf, 
           ...
           borderCol = NA) ## borderCol not always necessary, needed if plot is too white

ggsave("/Users/nataliefortunato/Documents/qbio_490_nataliefortunato/analysis_data/week7_maf/coOncoplot.png")
```
...
Hypothesized Reason: The GATA3 mutations may influence the expression profile of the tumor cells via impact on expression and activity rate of the GATA3 gene (Afzaljavan et. al.).
...
ggsave("/Users/nataliefortunato/Documents/qbio_490_nataliefortunato/analysis_data/week7_maf/lollipopPlot.png")
```
...
It is more commonly mutated in old patients.
- Are there more mutations in a specific region of the protein? Why might that be?
Most of the mutations occur in the early (gray) part of the gene. This may be because early mutations disrupt the gene expression more.
...
The vast majority of the mutations occur on the "grey region" of the lollipop plot.
...
I chose TP53. The mutant type (shown in red) is slightly higher than the wild type (blue).

What can you conclude from your plot?
Patients with the mutant TP53 gene have a slightly higher likely hood of survival up to 4000 days as compared to the wild type.

Is this what you would expect according to the literature? Why or why not?
I wouldn't expected this because TP53 is a very prevelent mutation found in cancer. One reason why the patients with this mutation have a higher rate of survival may be because there are treatments directed at TP53 because it is so well researched, or because it may be less harmful than other mutations.
